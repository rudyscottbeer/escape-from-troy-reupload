 ...
 */
public class Sprinkler extends WheelObstacle {
	 ...
	 */
	public Sprinkler(JsonValue data, float x, float y) {
	...
	public void bubble(Vector2 shipPos) {
		Particle p = memory.obtain();
		if (p != null) {
			p.position.set(shipPos);
			p.life = lifespan;
		}
	}
	 ...
	 */
	public void draw(GameCanvas canvas, Vector2 shipPos) {
		if (texture == null) {
			return;
		}
		...
		for(Particle p : memory.getPool()) {
			...
			if (p.life > 0) {
				canvas.draw(texture,c,origin.x,origin.y,
							p.position.x + ((canvas.getWidth() / 2) - shipPos.x),p.position.y + ((canvas.getHeight() / 2) - shipPos.y),0.0f,size,size);
			}
		}
	}
}
