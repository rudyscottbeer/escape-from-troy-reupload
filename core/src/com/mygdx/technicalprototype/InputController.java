...
public class InputController {
	...
	private Map<indexKeyToChange, Integer> keyToInput = new HashMap<>();
	...
	/** Whether the teritiary action button was pressed. (left click) */
	private boolean tertiaryPressed;
	/** Whether the quaternary action button was pressed. (right click) */
	...
	/** Whether the nearest planet resized button was pressed. */
	private boolean nearUpsizePressed;
	private boolean nearAPrevious;
	private boolean nearDownsizePressed;
	private boolean nearDPrevious;
	/** Whether any key was pressed (for title screen) */
	private boolean anyKeyPressed;
	private boolean anyKeyPrevious;
	 ...
	 */
	public boolean didPrimary() { return primePressed; }
	 ...
	 */
	public boolean didQuaternary() {
		return quaternaryPressed;
	}
	...
	/**
	 * Returns true if the nearest planet increased-size button was pressed.
	 *
	 * @return true if the nearest planet increased-size button was pressed.
	 */
	public boolean didNearA() {
		return nearUpsizePressed;
	}

	/**
	 * Returns true if the nearest planet decreased-size button was pressed.
	 *
	 * @return true if the nearest planet decreased-size button was pressed.
	 */
	public boolean didNearF() {
		return nearDownsizePressed;
	}

	public boolean didAnyKey() {
	...
	public void resetKeys() {
		keyToInput.put(indexKeyToChange.FORWARD, Input.Keys.D);
		keyToInput.put(indexKeyToChange.BACKWARD, Input.Keys.A);
		keyToInput.put(indexKeyToChange.INCREASE, Input.Keys.W);
		keyToInput.put(indexKeyToChange.DECREASE, Input.Keys.S);
		keyToInput.put(indexKeyToChange.RESTART, Input.Keys.R);
		keyToInput.put(indexKeyToChange.PAUSE, Input.Keys.ESCAPE);
	}

	public void setKeyForControl(indexKeyToChange key, int keyCode){
		keyToInput.put(key, keyCode);
	}

	public int getKeyForControl(indexKeyToChange key){
		return keyToInput.get(key);
	}
	...
	public void readInput(Rectangle bounds, Vector2 scale) {
		...
		anyKeyPressed = anyKeyPrevious;
		nearAPrevious = nearUpsizePressed;
		nearDPrevious = nearDownsizePressed;
	...
	private void readKeyboard(Rectangle bounds, Vector2 scale, boolean secondary) {
		...
		resetPressed = (secondary && resetPressed) || (Gdx.input.isKeyPressed(keyToInput.get(indexKeyToChange.RESTART)));
		debugPressed = (secondary && debugPressed) || (Gdx.input.isKeyPressed(Input.Keys.B));
    //Implement UP&DOWN and W&S control
		primePressed = (secondary && primePressed) ||
				(Gdx.input.isKeyPressed(keyToInput.get(indexKeyToChange.FORWARD)));
		secondPressed = (secondary && secondPressed) ||
				(Gdx.input.isKeyPressed(keyToInput.get(indexKeyToChange.BACKWARD)));

		prevPressed = (secondary && prevPressed) || (Gdx.input.isKeyPressed(Input.Keys.P));
		nextPressed = (secondary && nextPressed) || (Gdx.input.isKeyPressed(Input.Keys.N));
		exitPressed  = (secondary && exitPressed) || (Gdx.input.isKeyPressed(keyToInput.get(indexKeyToChange.PAUSE)));
		anyKeyPressed = (secondary && anyKeyPressed) || (Gdx.input.isKeyPressed(Input.Keys.ANY_KEY));
		nearUpsizePressed = (secondary && nearUpsizePressed) || (Gdx.input.isKeyPressed(keyToInput.get(indexKeyToChange.INCREASE)));
		nearDownsizePressed = (secondary && nearDownsizePressed) || (Gdx.input.isKeyPressed(keyToInput.get(indexKeyToChange.DECREASE)));
		...
		//Implement UP&DOWN and A&S control
		if ((Gdx.input.isKeyPressed(Input.Keys.UP))||(Gdx.input.isKeyPressed(Input.Keys.D))) {
			vertical += 1.0f;
		}
		...
		quaternaryPressed = (Gdx.input.isButtonPressed(Input.Buttons.RIGHT));
