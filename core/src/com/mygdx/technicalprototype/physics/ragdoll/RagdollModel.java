 ...
 */
package com.mygdx.technicalprototype.physics.ragdoll;
...
import com.mygdx.technicalprototype.Sprinkler;
import com.mygdx.technicalprototype.physics.obstacle.*;
...
public class RagdollModel extends ComplexObstacle {
    ...
    private final Sprinkler bubbler;
	...
	public RagdollModel(JsonValue data) {
	    ...
	    bubbler = new Sprinkler(data.get("bubbles"),ox,oy);
	}
     ...
     */
    public Sprinkler getBubbleGenerator() {
    	return bubbler;
    }
	...
	protected boolean createJoints(World world) {
		...
		float[] xOffs = new float[PART_RIGHT_SHIN+1];
		float[] yOffs = new float[PART_RIGHT_SHIN+1];

		for (int i = 0; i <= PART_RIGHT_SHIN; i++) {
			xOffs[i] = data.get( BODY_PARTS[partToAsset(i)] ).getFloat( 0 );
			yOffs[i] = data.get( BODY_PARTS[partToAsset(i)] ).getFloat( 1 );
		}

		RevoluteJointDef jointDef = new RevoluteJointDef();

		// values for placing the anchor points
		TextureRegion bodyTexture = partTextures[partToAsset(PART_BODY)];
		float bodyHeight = bodyTexture.getRegionHeight()/drawScale.y;
		float bodyWidth = bodyTexture.getRegionWidth()/drawScale.x;
		Vector2 armVecPos = new Vector2((partTextures[partToAsset(PART_LEFT_ARM)].getRegionWidth()/drawScale.x)/2, 0);
		Vector2 armVecNeg = new Vector2(-armVecPos.x, 0);
		Vector2 forearmVec = new Vector2((partTextures[partToAsset(PART_LEFT_FOREARM)].getRegionWidth()/drawScale.x)/2, 0);
		Vector2 thighVecPos = new Vector2(0, (partTextures[partToAsset(PART_LEFT_THIGH)].getRegionHeight()/drawScale.y)/2);
		Vector2 thighVecNeg = new Vector2(0, -thighVecPos.y);
		Vector2 shinVec = new Vector2(0, (partTextures[partToAsset(PART_LEFT_SHIN)].getRegionHeight()/drawScale.y)/2);

		// Body-head joint
		jointDef.bodyA = bodies.get(PART_BODY).getBody();
		jointDef.bodyB = bodies.get(PART_HEAD).getBody();
		jointDef.localAnchorA.set(new Vector2(0, bodyHeight/2));
		jointDef.localAnchorB.set(new Vector2(0, -(partTextures[partToAsset(PART_HEAD)].getRegionHeight()/drawScale.y)/2));
		...
		jointDef.bodyA = bodies.get(PART_BODY).getBody();
		jointDef.bodyB = bodies.get(PART_LEFT_ARM).getBody();
		jointDef.localAnchorA.set(new Vector2(-bodyWidth/2, yOffs[PART_LEFT_ARM]));
		jointDef.localAnchorB.set(armVecPos);
		jointDef.collideConnected = false;
		...
		// Body-right arm joint
		jointDef = new RevoluteJointDef();
		jointDef.bodyA = bodies.get(PART_BODY).getBody();
		jointDef.bodyB = bodies.get(PART_RIGHT_ARM).getBody();
		jointDef.localAnchorA.set(new Vector2(bodyWidth/2, yOffs[PART_RIGHT_ARM]));
		jointDef.localAnchorB.set(armVecPos);
		jointDef.collideConnected = false;
		jointDef.enableLimit = true;
		jointDef.lowerAngle = (float)-Math.PI/2;
		...
		// Body-left thigh joint
		jointDef = new RevoluteJointDef();
		jointDef.bodyA = bodies.get(PART_BODY).getBody();
		jointDef.bodyB = bodies.get(PART_LEFT_THIGH).getBody();
		jointDef.localAnchorA.set(new Vector2(-xOffs[PART_LEFT_THIGH], -bodyHeight/2));
		jointDef.localAnchorB.set(thighVecPos);
		jointDef.collideConnected = false;
		jointDef.enableLimit = true;
		jointDef.lowerAngle = (float)-Math.PI/2;
		jointDef.upperAngle =  (float)Math.PI/2;
		...
		// Body-right thigh joint
		jointDef = new RevoluteJointDef();
		jointDef.bodyA = bodies.get(PART_BODY).getBody();
		jointDef.bodyB = bodies.get(PART_RIGHT_THIGH).getBody();
		jointDef.localAnchorA.set(new Vector2(xOffs[PART_RIGHT_THIGH], -bodyHeight/2));
		jointDef.localAnchorB.set(thighVecPos);
		jointDef.collideConnected = false;
		jointDef.enableLimit = true;
		jointDef.lowerAngle = (float)-Math.PI/2;
		...
		// Left arm-left forearm joint
		jointDef = new RevoluteJointDef();
		jointDef.bodyA = bodies.get(PART_LEFT_ARM).getBody();
		jointDef.bodyB = bodies.get(PART_LEFT_FOREARM).getBody();
		jointDef.localAnchorA.set(armVecNeg);
		jointDef.localAnchorB.set(forearmVec);
		jointDef.collideConnected = false;
		jointDef.enableLimit = true;
		jointDef.lowerAngle = (float)-Math.PI/2;
		jointDef.upperAngle =  (float)Math.PI/2;
		...
		// Right arm-right forearm joint
		jointDef = new RevoluteJointDef();
		jointDef.bodyA = bodies.get(PART_RIGHT_ARM).getBody();
		jointDef.bodyB = bodies.get(PART_RIGHT_FOREARM).getBody();
		jointDef.localAnchorA.set(armVecNeg);
		jointDef.localAnchorB.set(forearmVec);
		jointDef.collideConnected = false;
		jointDef.enableLimit = true;
		...
		// Left thigh-left shin joint
		jointDef = new RevoluteJointDef();
		jointDef.bodyA = bodies.get(PART_LEFT_THIGH).getBody();
		jointDef.bodyB = bodies.get(PART_LEFT_SHIN).getBody();
		jointDef.localAnchorA.set(thighVecNeg);
		jointDef.localAnchorB.set(shinVec);
		jointDef.collideConnected = false;
		jointDef.enableLimit = true;
		jointDef.lowerAngle = (float)-Math.PI/2;
		jointDef.upperAngle =  (float)Math.PI/2;
		...
		// Right thigh-right shin joint
		jointDef = new RevoluteJointDef();
		jointDef.bodyA = bodies.get(PART_RIGHT_THIGH).getBody();
		jointDef.bodyB = bodies.get(PART_RIGHT_SHIN).getBody();
		jointDef.localAnchorA.set(thighVecNeg);
		jointDef.localAnchorB.set(shinVec);
		jointDef.collideConnected = false;
		jointDef.enableLimit = true;
